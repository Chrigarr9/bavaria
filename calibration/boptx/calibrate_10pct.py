...
POPULATION = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", "output", "populations", "population_10pct_kelheim30km.xml.gz"))
WORKING_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "work_10pct")

ITERATIONS = 100  # Enough for 1% convergence with DMC annealing
...
# Parameters: ASCs with Kelheim v3.0 calibrated values as initial, bounds ±2 around them.
# Walk ASC fixed at 0 (reference mode) — calibrate car, bike, pt relative to walk.
# Kelheim ASCs: car=0.109, bike=-0.906, pt=0.045, walk=0.0
parameters = [
    AscParameter("car", bounds=(0.2878, 2.2878), initial_value=1.2878),
    AscParameter("bike", bounds=(-1.0948, 0.9052), initial_value=-0.0948),
    AscParameter("pt", bounds=(-2.1873, -0.1873), initial_value=-1.1873),
    AscParameter("walk", bounds=(-0.6048, 1.3952), initial_value=0.3952),
]
...
class BaseSimEvaluator(BaseEvaluator):
    ...
    def start_evaluation(self, identifier, values, information):
        ...
        exec_args = " ".join([
            ...
            "--capacity", "10",     # freeflow — 1% population won't congest anyway
    ...
    def get_evaluation(self, identifier):
        ...
        # Weighted sum: mode shares primary, flow secondary
        obj = 1.0 * mode_result["objective"] + 0.3 * flow_result["objective"]
...
if __name__ == "__main__":
    ...
    tracker = PickleTracker("optimization_10pct.p")
